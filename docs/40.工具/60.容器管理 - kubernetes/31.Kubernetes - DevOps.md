...
填写好用户名密码后，需要注意凭证 id 要与 Jenkinsfile 中的 **\*DOCKER_CREDENTIAL_ID\*** _一致_

**Gitlab 访问凭证**

通过系统管理 > Manage Credentials > 凭据 > System > 全局凭证 > Add Credentials 添加 Username with password 类型凭证

填写好用户名密码后，需要注意凭证 id 要与 Jenkinsfile 中的 `GIT_CREDENTIAL_ID` _一致_
...
6. 复制保存后文件 id 到 Jenkinsfile 中的 `KUBECONFIG_CREDENTIAL_ID` _处_
...
4. 保证凭证 id 与 Jenkinsfile 文件中的 `SONAR_CREDENTIAL_ID` _一致_
